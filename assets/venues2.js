...
fetch(url)
  ...
  .then(data => {
    ...
    results.forEach(row => {
      ...
      const techInfo = row['Technical information'] ? `<strong>Technical Information:</strong><br>${row['Technical information'].split('\n').join('</p><p>')}<br>` : '';
      infoCell.innerHTML = `${row['Description'].split('\n').join('</p><p>')}<br>${techInfo}<strong>Contacts:</strong><br>${row['Contacts'].split('\n').join('</p><p>')}`;
