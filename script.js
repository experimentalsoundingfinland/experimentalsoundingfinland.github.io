        ...
        data.items.forEach((event) => {
            ...
            if (eventEndDateTime > now) {
                ...
                if (description.includes(venuePrefix)) {
                    ...
                    const venueAndRest = splitDescription[1].split(' ');
                    const venueIndex = venueAndRest.findIndex(word => word === '');
                    venue = venueAndRest.slice(0, venueIndex).join(' ');
                    description = splitDescription[0] + ' ' + venueAndRest.slice(venueIndex + 1).join(' ');
                }
